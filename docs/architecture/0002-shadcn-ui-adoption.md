...
4. Integrate only after package and application checks pass, then obtain the
   required technical and human UI review evidence before merging.
...
branch is `feature/factory-shadcn-ui`. Implementers may push or open pull
requests but must not merge.

## Rollback

The migration is incremental and reversible. Revert the affected task commit or
pull request to restore the preceding package exports, CSS imports, and
app-local markup. If a shared primitive causes a regression, applications may
