...
function comparisonLocation(index: number): Location {
  return {
    ...locations[0]!,
    id: `comparison-${index}`,
...
function signalAt(mock: ReturnType<typeof vi.fn>, index: number): AbortSignal {
  return (mock.mock.calls[index]?.[1] as { signal: AbortSignal }).signal;
}
...
describe("useWeatherComparison", () => {
  it("adds a location in loading state and replaces only it when its request succeeds", async () => {
    const request = deferred<CurrentConditions>();
    const getCurrentConditions = vi.fn().mockReturnValue(request.promise);
    const dataSource: WeatherComparisonDataSource = { getCurrentConditions };
    const { result } = renderHook(() => useWeatherComparison(dataSource));

    let outcome: ReturnType<typeof result.current.addLocation>;
    act(() => {
  ...
  it("keeps entries independent when later requests settle before earlier failures", async () => {
    const first = deferred<CurrentConditions>();
    const second = deferred<CurrentConditions>();
    const getCurrentConditions = vi
      .fn()
  ...
  it("retries only a failed location with a new request and ignores prior provider details", async () => {
    const failed = deferred<CurrentConditions>();
    const second = deferred<CurrentConditions>();
    const retry = deferred<CurrentConditions>();
    const getCurrentConditions = vi
      .fn()
