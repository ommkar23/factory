...
class Location(ApiModel):
    countryCode: str = Field(description="ISO 3166-1 alpha-2 country code.", examples=["US"])
    id: str = Field(description="Stable provider location identifier.", examples=["5746545"])
    latitude: float = Field(description="WGS84 latitude in decimal degrees.", examples=[45.5234])
    longitude: float = Field(description="WGS84 longitude in decimal degrees.", examples=[-122.6762])
    ...
    timezone: str = Field(description="IANA time-zone identifier for the location.", examples=["America/Los_Angeles"])


class CurrentConditions(ApiModel):
    apparentTemperatureC: float = Field(description="Apparent temperature in degrees Celsius.")
    ...
    observedAt: str = Field(description="Provider-local ISO 8601 observation timestamp.")
    ...
    windDirectionDegrees: float = Field(description="Wind direction in degrees clockwise from north.")
...
class LocationsResponse(ApiModel):
    locations: list[Location] = Field(description="At most five normalized location matches.")
