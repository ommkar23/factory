...
def test_state_store_encrypts_and_consumes_browser_pkce_state_once(
    tmp_path: Path,
) -> None:
    database_url = "sqlite:///" + str(tmp_path / Path("states.db"))
    store = session_store.SqliteSessionStore(
        database_url, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="
    )
    store.store_oauth_state("state", "verifier", "/weather", "binding")
    state = store.consume_oauth_state("state", "binding")
    assert state is not None
    assert state.next_path == "/weather"
